...
[[용어정리#^aa482c|Trade-Off]] : 하나를 선택하면 다른 것은 포기해야하는 상황
...
##### 그리드 근사(Grid approximation)


 사후 추론의 가장 단순한 근사. 

 베이지안 추론의 핵심은 사후분포를 계산하는 것. 
 사후 분포 $p(\theta|D) = \frac{p(D|\theta)}{p(D)}$

 하지만 $P(D) = \int p(D|\theta)p(\theta)d\theta$가 고차원적이라 적분이 불가능. 즉, 근사가 필요.

연속 공간을 이산 Grid로 쪼개서 적분을 합으로 대체.

 $\int f(\theta)d\theta \approx \sum_k f(\theta_k)\Delta\theta$   $\Delta\theta$는 그리드 간격으로 작을 수록 더욱 정확해짐.


$p(\theta=\theta_k)|D) \approx \frac{p(D|\theta_k)p(\theta_k)}{p(D)} =  \frac{p(D|\theta_k)p(\theta_k)}{\sum_{k'=1}^K p(D,\theta_{k'})}$

 p(D)를 근사한 것. $\hat p(D) = \sum_{k=1}^Kp(D,\theta_k) \Delta\theta$ 위의 식에선 그리드간격이 1인 근사.

##### 라플라스 근사(Laplace approximation)

 라플라스근사는 2차근사라고도 부름.

 [[용어정리#^8fbb80|테일러 전개]]: 어떤 함수 f(x)를 점 a 주위에서 다항식으로 근사하는 방법.
  2차 테일러전개는 점 a 주위에 2차함수모양으로 근사한 것.

 사후분포의 Mode 주위에 2차 테일러 전개를 통해 포물선 모양으로 근사함.
 그렇게 근사한 결과를 사후분포로 사용.
 이 사후분포는 MAP추정으로 구해서 사용.

##### Variational approximation

라플라스 근사보다 더 많은 모델링 유연성과, 더 정확한 근사를 제공.
tractable한 분포로 근사한 후, 분포들간의 차이를 최소화



##### Markov Chain Monte Carlo (MCMC) approximation

  마르코프 체인과 몬테 카를로 근사를 결합하여 사후 분포를 근사.
...
### 신뢰구간
  ...
  데이터를 반복해서 생성하여 구간들을 생성.
 이 것을 반복하면 구간들 중 95%가 $\theta^*$, 정답 파라미터를 포함한다는 의미.
...
#### 편향
 ...
 bias가 0이면 비편향이라고 말함. 파라미터가 정답과 일치.
